...
{
    public class StoreUser : IUserStore<ApplicationUser>, IUserEmailStore<ApplicationUser>, IUserPhoneNumberStore<ApplicationUser>,
    IUserTwoFactorStore<ApplicationUser>, IUserPasswordStore<ApplicationUser>
    {
        private IConfiguration _config;
        private string _strAPIUrl;
        public StoreUser(IConfiguration iconfig)
        {
            _config = iconfig;
        ...
        public async Task<IdentityResult> CreateAsync(ApplicationUser user, CancellationToken cancellationToken)
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                HttpResponseMessage response = client.GetAsync("/api/Global/GetSrcCalcDevident?Col1=").Result;
                using (HttpContent content = response.Content)
                {
                    string stringData = await content.ReadAsStringAsync();
                    user = JsonConvert.DeserializeObject<ApplicationUser>(stringData);
                }
            }
            ...
            public async Task<ApplicationUser> FindByIdAsync(string userId, CancellationToken cancellationToken)
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    {
                        string stringData = await content.ReadAsStringAsync();
                        return user = JsonConvert.DeserializeObject<ApplicationUser>(stringData);
                    }
                }
