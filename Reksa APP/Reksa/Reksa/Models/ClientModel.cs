using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace Reksa.Models
{
    public class ClientModel
    {
        public int? ClientId { get; set; }
        public string ProdCode { get; set; }
        public string ClientCode { get; set; }
        public string ClientName { get; set; }
        public string CIFNo { get; set; }
        public string CIFName { get; set; }
        public string CIFAddress1 { get; set; }
        public string CIFAddress2 { get; set; }
        public string CIFAddress3 { get; set; }
        public string CIFCity { get; set; }
        public string CIFProvince { get; set; }
        public string CIFCountryCode { get; set; }
        public string CIFTelephone { get; set; }
        public string CIFHP { get; set; }
        public string CIFBirthPlace { get; set; }
        public string CIFBirthDay { get; set; }
        public string CIFIDNo { get; set; }
        public string CIFNPWP { get; set; }
        public string CIFStatus { get; set; }
        public string IsEmployee { get; set; }
        public string CIFType { get; set; }
        public string CIFNIK { get; set; }
        public string ShareholderID { get; set; }
        public string Flag { get; set; }
        public string NamaNPWP { get; set; }
        public string TglNPWP { get; set; }
        public string NoNPWPKK { get; set; }
        public string NamaNPWPKK { get; set; }
        public string KepemilikanNPWPKK { get; set; }
        public string KepemilikanNPWPKKLainnya { get; set; }
        public string TglNPWPKK { get; set; }
        public string AlasanTanpaNPWP { get; set; }
        public string NoDokTanpaNPWP { get; set; }
        public string TglDokTanpaNPWP { get; set; }
        public string Opsi { get; set; }
        public string NoNPWPProCIF { get; set; }
        public string NamaNPWPProCIF { get; set; }
        public string SII { get; set; }
        public string JoinDate { get; set; }
        public string AgentCode { get; set; }
        public string AccountType { get; set; }
        public string BankCode { get; set; }
        public string NISPAccId { get; set; }
        public string NonNISPAccId { get; set; }
        public string NonNISPAccName { get; set; }
        public string Referentor { get; set; }
        public string DocRiskProfile { get; set; }
        public string DocTermCondition { get; set; }
        public string Inputter { get; set; }
        public string Seller { get; set; }
        public string Waperd { get; set; }
        public string RiskProfile { get; set; }
        public string LastUpdate { get; set; }
        public string IsRegistered { get; set; }
        public string ExpRiskProfileYear { get; set; }
        public string Status { get; set; }
        public string LastUser { get; set; }
        public string EditDate { get; set; }
        public string JangkaWaktu { get; set; }
        public string JatuhTempo { get; set; }
        public string AutoRedemption { get; set; }
        public string Asuransi { get; set; }

    }

    public class TransaksiClientNew
    {
        public string ClientCode { get; set; }
        public string ClientName { get; set; }
        public string ClientId { get; set; }
        public string CIFNo { get; set; }
        public DateTime JoinDate { get; set; }
        public bool IsEmployee { get; set; }
        public string IsRDB { get; set; }
    }
}
