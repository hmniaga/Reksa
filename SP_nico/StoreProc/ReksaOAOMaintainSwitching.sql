...
CREATE proc [dbo].[ReksaOAOMaintainSwitching]            
/*            
 CREATED BY    : liliana            
 CREATION DATE :             
...
END                  
 ...
 set @mTotalFeeBased = isnull(@mSwcFeeBased, 0) + isnull(@mTaxFeeBased,0) + isnull(@mFeeBased3,0)+ isnull(@mFeeBased4,0)+ isnull(@mFeeBased5,0)        
   ...
   --cek apakah sudah ada subscription new / trx switch in blm               
    if @pbTrxTaxAmnesty = 1    
    begin    
        set @nExtStatus = 74    
    end    
...
if isnull(@cSII,'') = ''    
begin    
	insert CIFPendingTransaction_TT (
    ...
    where CIFNo = @pcCIFNo
	  ...
	  insert dbo.CIFPendingTransactionSwcReksadana_TT (TranCode, TranType, TranDate, ProdSwitchOut, ProdSwitchIn,         
