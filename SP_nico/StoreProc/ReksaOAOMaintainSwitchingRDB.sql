...
CREATE proc [dbo].[ReksaOAOMaintainSwitchingRDB]      
/*      
 CREATED BY    : liliana      
 CREATION DATE :     
 DESCRIPTION   :  khusus online account opening yg tidak punya SII        
 REVISED BY    :      
...
@pnTranType                 int,  -- 5: switching sebagian ; 6: switching all  ; 9 : switching RDB    
...
, @dJatuhTempo              datetime  
, @cTempJatuhTempo          varchar(20)  
, @cDefaultCCY              char(3)  
, @nBankId                  int   
, @nAgentId                 int  
, @pnFundIdSwcOut           int  
...
set @cTrxType = 'SWC'  
set @cTempJatuhTempo = left(convert(varchar,@pdJatuhTempo),4) + '-' + substring(convert(varchar,@pdJatuhTempo),5,2) + '-' + right(convert(varchar,@pdJatuhTempo),2)  
set @dJatuhTempo = convert(datetime,@cTempJatuhTempo)   
set @pbIsJatuhTempo = 0  
...
set @nCIF = convert (bigint, @pcCIFNo)   
...
begin
    if @nCurrSwcOut = 'IDR' and isnull(@cNoRekeningTA,'') != ''   
    begin   
        set @pcSelectedAccNo = @cNoRekeningTA  
    end  
    else if @nCurrSwcOut = 'USD' and isnull(@cNoRekeningUSDTA,'') != ''  
    begin  
...
begin
if @nCurrSwcOut = 'IDR' and isnull(@pcSelectedAccNo,'') != ''   
begin   
    set @pcSelectedAccNo = @pcSelectedAccNo  
end  
else if @nCurrSwcOut = 'USD' and isnull(@cNoRekeningUSD,'') != ''  
begin  
...
begin  
 ...
 begin  
  ...
  goto ERROR  
    ...
    begin      
        ...
        select @CIFNIK = CIFNik   
        from ReksaMasterNasabah_TM where CIFNo = @cCIFNo   
 ...
 If @pbIsNew = 0 and not exists(select top 1 1 from ReksaCIFData_TM where ClientId = @pnClientIdSwcIn and CIFStatus = 'A')           
    ...
    if @pnAutoRedemption = 1 and @dJatuhTempo = @dCurrDate  
    begin  
        set @cErrMsg = 'Proses switching RDB tidak dapat dilakukan karena AutoRedemption aktif dan client code switch out jatuh tempo hari ini !'              
        goto ERROR        
    end  
    ...
    if exists(select top 1 1 from dbo.ReksaRegulerSubscription_TR where ProductId  = @pnProdIdSwcOut 
        and SwcFeeNonAsuransi is null
        )
    begin
        select @cErrMsg = 'Harap lakukan setting % Switching Fee Non Asuransi utk produk ' + ProdCode + ' !' from ReksaProduct_TM where ProdId = @pnProdIdSwcOut           
        goto ERROR      
    end
    
    if exists(select top 1 1 from dbo.ReksaRegulerSubscription_TR where ProductId  = @pnProdIdSwcOut 
        and SwcFeeAsuransi is null
        )
    begin
        select @cErrMsg = 'Harap lakukan setting % Switching Fee Asuransi utk produk  ' + ProdCode + ' !' from ReksaProduct_TM where ProdId = @pnProdIdSwcOut           
        goto ERROR      
    end 
    ...
    if @cCurrencyType != @nCurrSwcOut  
    Begin              
        set @cErrMsg = 'Transaksi switching harus dalam mata uang yang sama dengan mata uang produk!'              
        goto ERROR             
    End   
    ...
    set @pmTranAmt= @pmTranUnit * @mLastNAV  
  
    if exists(select top 1 1 from dbo.ReksaRegulerSubscriptionSchedule_TT  
    where TranId = @nTranIdOld  
    and StatusId in (0,3)  
    )  
    and isnull(@pnJangkaWaktu,0) != 0  
    begin  
        ...
        begin  
            select @cErrMsg = 'Client Code RDB yang belum jatuh tempo harus Switching All'         
            goto ERROR   
        end   
        ...
        begin  
            ...
            begin     
                ...
                begin    
                    ...
                    begin       
                       set @cErrMsg = 'Minimal Switching RDB: ' + convert(varchar(40),cast(@mMinSwitchRedempt as money),1)                             
                       goto ERROR                
                    end      
                end    
                ...
                begin    
                    ...
                    begin             
                        set @cErrMsg = 'Minimal Switching RDB: ' + convert(varchar(40),cast(@mMinSwitchRedempt as money),1) + ' Unit Perkiraan'                              
                        goto ERROR                
                    end      
                end    
            end     
          ...
          begin                      
                        set @cErrMsg = 'Minimal Switching RDB: ' + convert(varchar(40),cast(@mMinSwitchRedempt as money),1) + ' (Perkiraan)'                                
                        goto ERROR                
                    end      
                end    
                else if(@cJenisSwitchRedempt = 'Unit')    
                begin    
                    ...
                    begin            
                        set @cErrMsg = 'Minimal Switching RDB: ' + convert(varchar(40),cast(@mMinSwitchRedempt as money),1) + ' Unit'                           
                        goto ERROR                
                    end      
                end    
            end   
...
begin  
  ...
  --total fee based  
    ...
    begin          
      ...
      exec ReksaGenerateRefID 'SWCRDB', @pcRefID output    
      ...
      begin  
        ...
        begin  
            ...
            if @pbIsJatuhTempo = 0  
            begin            
            insert dbo.ReksaRegulerSubscriptionClient_TM (ClientId, Status, LastUser, LastUpdate)  
            select @pnClientIdSwcIn, 1, 7, getdate()    
            end  
        ...
        begin  
            set @pnJangkaWaktu = 0  
            set @dJatuhTempo = null  
            set @pnFrekPendebetan = 0  
            set @pnAutoRedemption = null  
            set @pnAsuransi = null  
        end                  
	...
	if isnull(@cSII,'') = ''  
	begin  
		insert dbo.CIFPendingTransaction_TT (
		NoCIF, NoSecurity, NamaNasabah, TanggalInput, JenisInvestasi, Product, AlamatNasabah, NoTlpn, StatusProcess
		)
		select @pcCIFNo, @pcTranCode, CIFName, getdate(), 'REKSADANA', @cProdCodeSwcOut, '', '', null 
		from dbo.ReksaMasterNasabah_TM        
		where CIFNo = @pcCIFNo
	...
	begin           
         insert dbo.ReksaSwitchingTransaction_TM (TranCode, TranType, TranDate, ProdSwitchOut, ProdSwitchIn,   
            ...
            , Channel  
            , RefID, OfficeId, Referentor, JangkaWaktu, JatuhTempo, FrekPendebetan, AutoRedemption, Asuransi, IsNew  
            , AuthType   
         ...
         select @pcTranCode, @pnTranType, @pdTranDate, @pnProdIdSwcOut, @pnProdIdSwcIn,   
             @pnClientIdSwcOut, @pnClientIdSwcIn, @pnFundIdSwcOut, @pnFundIdSwcIn, @pcSelectedAccNo, @nAgentId,     
             0, @nCurrSwcOut, @pmTranUnit * @pmNAVSwcOut, @pmTranUnit, @pmSwitchingFee, @pmNAVSwcOut, @pmNAVSwcIn, convert(char(8),@pdNAVValueDate,112),                   
             @pmUnitBalanceSwcOut, @pmUnitBalanceSwcOut * @pmNAVSwcOut, @pmUnitBalanceSwcOut * @pmNAVSwcOut / @pmNAVSwcIn, @pmUnitBalanceSwcOut * @pmNAVSwcOut, 0, @pbByUnit,  
            ...
            , @cChannel  
            , @pcRefID, @pcOfficeId, @pnReferentor, @pnJangkaWaktu, @dJatuhTempo, @pnFrekPendebetan, @pnAutoRedemption, @pnAsuransi, @pbIsNew   
            , 1          
    ...
    begin   
        ...
        insert dbo.ReksaSwitchingTransaction_TMP (TranCode, TranType, TranDate, ProdSwitchOut, ProdSwitchIn,   
            ...
            , SwitchingFeeBased, TaxFeeBased, FeeBased3, FeeBased4, FeeBased5, TotalFeeBased  
            , Channel  
            , RefID, OfficeId, Referentor, JangkaWaktu, JatuhTempo, FrekPendebetan, AutoRedemption, Asuransi, IsNew, AuthType  
            ,DocFCSubscriptionForm                  
            ,DocFCDevidentAuthLetter                 
            ,DocFCJoinAcctStatementLetter            
            ,DocFCIDCopy                             
         ...
         select @pcTranCode, @pnTranType, @pdTranDate, @pnProdIdSwcOut, @pnProdIdSwcIn,  
            ...
            , @mSwcFeeBased, @mTaxFeeBased, @mFeeBased3, @mFeeBased4, @mFeeBased5, @mTotalFeeBased  
            , @cChannel  
            , @pcRefID, @pcOfficeId, @pnReferentor, @pnJangkaWaktu, @dJatuhTempo, @pnFrekPendebetan, @pnAutoRedemption, @pnAsuransi, @pbIsNew, 4        
            ,@pbDocFCSubscriptionForm                  
            ,@pbDocFCDevidentAuthLetter                 
            ,@pbDocFCJoinAcctStatementLetter            
            ,@pbDocFCIDCopy                             
